...
//m.j.y 
#define GPIO_DIR_CTRL_OUTPUT  gpio_direction_output

//#define GPIO_DIR_CTRL_OUTPUT
...
static int uart3_rs485_en_gpio;
static int uart4_rs485_en_gpio;
static int uart5_rs485_en_gpio;
//static int uart6_rs485_en_gpio;
...
static void imx_stop_rx(struct uart_port *port)
{
	...
	if (sport->dma_is_enabled && sport->dma_is_rxing) {
		if (sport->port.suspended) {
			...
			//	printk("mjy--port suspend\n");
		}
	 else
	 {
			GPIO_DIR_CTRL_OUTPUT(uart3_rs485_en_gpio,1);
			GPIO_DIR_CTRL_OUTPUT(uart4_rs485_en_gpio,1);
			GPIO_DIR_CTRL_OUTPUT(uart5_rs485_en_gpio,1); //2019-03-18 m.j.y
		//	GPIO_DIR_CTRL_OUTPUT(uart6_rs485_en_gpio,1);
		//	printk("mjy--imx_stop_rx, gpio--hign\n");
			return;
		}
	}
...
static void dma_tx_callback(void *data)
{
	...
	//M.J.Y 2019-03-19
	 if ((2 == sport->port.line) || 	(3 == sport->port.line) || 	(4 == sport->port.line) /* || 	(5 == sport->port.line)*/ )
	{
		temp = readl(sport->port.membase + UCR4);
		temp |= UCR4_TCEN;
		writel(temp, sport->port.membase + UCR4);
...
static void dma_tx_work(struct work_struct *w)
{
	...
	if (sport->tx_bytes > 0) {
		if(2 == sport->port.line)
		{
			GPIO_DIR_CTRL_OUTPUT(uart3_rs485_en_gpio,1);
		//	printk("mjy--tx call 1\n");
		}
		if(3 == sport->port.line)
		{
			GPIO_DIR_CTRL_OUTPUT(uart4_rs485_en_gpio,1);
		}
		if(4 == sport->port.line)
		{
			GPIO_DIR_CTRL_OUTPUT(uart5_rs485_en_gpio,1);
		}
		//2019-03-18 m.j.y
	/*	if(5 == sport->port.line)
		{
			GPIO_DIR_CTRL_OUTPUT(uart6_rs485_en_gpio,1);
		}
		*/
  ...
  if ((2 == sport->port.line) || 	(3 == sport->port.line) || 	(4 == sport->port.line) || 	(5 == sport->port.line) )
	...
	*/
	...
	if(2 == sport->port.line)
	{
		...
		udelay(5);
		GPIO_DIR_CTRL_OUTPUT(uart3_rs485_en_gpio,0);
		//	printk("mjy--tx err,uart3 low\n");
	}
	if(3 == sport->port.line)
	{
		/* in rs485 mode disable transmitter if shifter is empty */
		while (!(readl(sport->port.membase + USR2) & USR2_TXDC));
		udelay(5);
		GPIO_DIR_CTRL_OUTPUT(uart4_rs485_en_gpio,0);
	//	printk("mjy--tx err,uart4 low\n");
	}
	if(4 == sport->port.line)
	{
		/* in rs485 mode disable transmitter if shifter is empty */
		while (!(readl(sport->port.membase + USR2) & USR2_TXDC));
		udelay(5);
		GPIO_DIR_CTRL_OUTPUT(uart5_rs485_en_gpio,0);
	//	printk("mjy--tx err,uart5 low\n");
	}
	if(5 == sport->port.line)
	{
		/* in rs485 mode disable transmitter if shifter is empty */
		while (!(readl(sport->port.membase + USR2) & USR2_TXDC));
		udelay(5);
		GPIO_DIR_CTRL_OUTPUT(uart6_rs485_en_gpio,0);
	//	printk("mjy--tx err,uart6 low\n");
	}
#endif
}
...
static irqreturn_t imx_txint(int irq, void *dev_id)
{
	...
	if (2 == sport->port.line)
	{
		...
		if  ((readl(sport->port.membase + USR2) & USR2_TXDC))
		{
		    ...
		    GPIO_DIR_CTRL_OUTPUT(uart3_rs485_en_gpio,0);
	//	printk("intr, uart3 low\n");
		goto out;
	  }
	}
	else if (3 == sport->port.line)
	{
		/* in rs485 mode disable transmitter if shifter is empty */
		if  ((readl(sport->port.membase + USR2) & USR2_TXDC))
			{
			...
			GPIO_DIR_CTRL_OUTPUT(uart4_rs485_en_gpio,0);
	//	printk("intr, uart4 low\n");
		goto out;
	  }
	}
	else if (4 == sport->port.line)
	{
		/* in rs485 mode disable transmitter if shifter is empty */
		if  ((readl(sport->port.membase + USR2) & USR2_TXDC))
			{
				...
				writel(temp, sport->port.membase + UCR4);
			GPIO_DIR_CTRL_OUTPUT(uart5_rs485_en_gpio,0);
	//	printk("intr, uart5 low\n");
		goto out;
	  }
	}
	//2019-03-19
	/*else if (5 == sport->port.line)
	{
		// in rs485 mode disable transmitter if shifter is empty 
		if  ((readl(sport->port.membase + USR2) & USR2_TXDC))
			{
...
//	printk("mjy:current portline=%d\n", sport->port.line);
	if(2 == sport->port.line)
	{
		gpio_direction_output(uart3_rs485_en_gpio,0);//启动串口时处于接收状态
	//	printk("mjy--uart3 gpio low\n");
	}
	if(3 == sport->port.line)
	{
		gpio_direction_output(uart4_rs485_en_gpio,0);//启动串口时处于接收状态
	//	printk("mjy--uart4 gpio low\n");
	}
	if(4 == sport->port.line)
	{
		gpio_direction_output(uart5_rs485_en_gpio,0);//启动串口时处于接收状态
	//	printk("mjy--uart5 gpio low\n");
	}
	//2019-03-19
/*	if(5 == sport->port.line)
	{
		gpio_direction_output(uart6_rs485_en_gpio,0);//启动串口时处于接收状态
	//	printk("mjy--uart6 gpio low\n");
	}
	*/
		...
		struct platform_device *pdev)
{
	...
	if(2==sport->port.line)
	{
		pr_info("Serial: RS485 Gpio Init\n");
		/* request power down pin */
		uart3_rs485_en_gpio = of_get_named_gpio(np, "rs485-gpios", 0);
		
		if (gpio_is_valid(uart3_rs485_en_gpio)) 
		{
			ret = gpio_request(uart3_rs485_en_gpio, "serial");
			if (ret < 0)
			return ret;
		}
		else
		{
			pr_info("no sensor rs485_en pin available\n");
		}
	}
	if(3==sport->port.line)
	{
		pr_info("Serial: RS485 Gpio Init\n");
		/* request power down pin */
		uart4_rs485_en_gpio = of_get_named_gpio(np, "rs485-gpios", 0);
		
		if (gpio_is_valid(uart4_rs485_en_gpio)) 
		{
			ret = gpio_request(uart4_rs485_en_gpio, "serial");
			if (ret < 0)
			return ret;
		}
		else
	...
	if(4==sport->port.line)
	{
		pr_info("Serial: RS485 Gpio Init\n");
		/* request power down pin */
		uart5_rs485_en_gpio = of_get_named_gpio(np, "rs485-gpios", 0);
		
		if (gpio_is_valid(uart5_rs485_en_gpio)) 
		{
			ret = gpio_request(uart5_rs485_en_gpio, "serial");
			if (ret < 0)
			return ret;
		}
		else
...
/*	if(5==sport->port.line)
	{
		pr_info("Serial: RS485 Gpio Init\n");
		// request power down pin 
		uart6_rs485_en_gpio = of_get_named_gpio(np, "rs485-gpios", 0);
		
		if (gpio_is_valid(uart6_rs485_en_gpio)) 
		{
			ret = gpio_request(uart6_rs485_en_gpio, "serial");
			if (ret < 0)
			return ret;
		}
		else
