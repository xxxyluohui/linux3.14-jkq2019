...
static int fec_enet_mii_probe(struct net_device *ndev)
{
	...
	if (fep->phy_node) {
		...
		printk("mjy enter probe, node=%x \n", fep->phy_node);
		phy_dev = of_phy_connect(ndev, fep->phy_node,
					 &fec_enet_adjust_link, 0,
					 fep->phy_interface);
		
		//add by m.j.y	
		while ((!phy_dev)	&& (nRestTimes > 0))
		//if (!phy_dev)	
		{
			nRestTimes--;
			printk("scan No.%d time\n", nRestTimes);
				phy_dev = of_phy_connect(ndev, fep->phy_node,
					 &fec_enet_adjust_link, 0,
					 fep->phy_interface);
		}		 
	...
	else  
	 {
		...
		//M.J.Y 
		netdev_info(ndev, "M.J.Y phy_id=%08X \n", phy_id);
	}
	...
	netdev_info(ndev, "MJY Freescale FEC PHY driver [%s] (mii_bus:phy_addr=%s, irq=%d)\n",
...
static int fec_enet_mii_init(struct platform_device *pdev)
{
	 ...
	 * fec0 mii_bus.
	 */
	 printk("cur fec dev_id=%x", fep->dev_id);
	...
	if ((fep->quirks & FEC_QUIRK_ENET_MAC) && fep->dev_id > 0) {
		...
		if (mii_cnt && fec0_mii_bus) {
			...
			*fec_mii_bus_share = FEC0_MII_BUS_SHARE_TRUE;
			...
			//ADD MY M.J.Y 2017-11-22 再次查询一次各个phy
		/*	if (fep->mii_bus->irq)
			{
  ...
  printk("will call mdiobus_register\n");
	node = of_get_child_by_name(pdev->dev.of_node, "mdio");
	if (node) {
		err = of_mdiobus_register(fep->mii_bus, node);
		of_node_put(node);
...
static void fec_reset_phy(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(phy_reset))
		{
			//printk("m.j.y not valid phy_reset\n");
		return;
	}
...
fec_probe(struct platform_device *pdev)
{
 ...
 // printk("call fec_probe\n");
	...
	ret = fec_enet_clk_enable(ndev, true);
	//printk("call fec_enet_clk_enable true");
	if (ret)
		goto failed_clk;
