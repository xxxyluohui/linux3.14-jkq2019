...
static int mdio_bus_match(struct device *dev, struct device_driver *drv)
{
	...
	rev =  (phydrv->phy_id & phydrv->phy_id_mask) ==
		(phydev->phy_id & phydrv->phy_id_mask);
		
		if (rev)
			  printk("--- match£¬phy_id = %x, phy_id_mask=%x, DRV_NAME=%s\n", phydev->phy_id, phydrv->phy_id_mask, phydrv->name );
			//  else
			  	//  printk("m.j.y-------not match£¬phy_id = %x, phy_id_mask=%x, DRV_NAME=%s\n", phydev->phy_id, phydrv->phy_id_mask, phydrv->name);
